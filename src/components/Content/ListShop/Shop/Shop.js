...
class Shop extends React.Component {
    ...
    getMainImage() {
        return fetch(`http://localhost:4000/api/v1/shops/get-main-image`, {
            method: 'GET',
            headers: {
    ...
    getSupportImages() {
        return fetch(`http://localhost:4000/api/v1/shops/get-support-images`, {
            method: 'GET',
            headers: {
                idshop: this.props.shop.id
            }
        ...
        return (
            <Row className='shop-item'>
                ...
                <Col md={3} className='shop-image'>
                    <img src={`http://localhost:4000/image/${this.state.mainShopImage.path}/${this.state.mainShopImage.name}`} alt="" />
                </Col>
                        ...
                        {this.state.spShopImages && this.state.spShopImages.map((spImage, key) => {
                            return (
                                <div className='image-item' key={key}>
                                    <img src={`http://localhost:4000/image/${spImage.path}/${spImage.name}`} id={`image-${spImage.id}`} alt=""
                                    ...
                                    <div className='zoom-img' id={`zoom-image-${spImage.id}`}>
                                        <img src={`http://localhost:4000/image/${spImage.path}/${spImage.name}`} alt="" />
                                    </div>
                                </div>
                            );
                        })}
