...
class BookComments extends React.Component {
    ...
    getAccComment() {
        return fetch(`http://localhost:4000/api/v1/books/book-name-comment`, {
            method: 'GET',
    ...
    getAccAvt() {
        return fetch(`http://localhost:4000/api/v1/accounts/get-image`, {
            method: 'GET',
            headers: { accountid: this.props.comment.create_by }
        }).then(response => { return response.json() }).then(responseJson => {
        ...
        return (
            <div className='comment-item'>
                <div className='avt-cmt'>
                    <img src={`http://localhost:4000/image/${this.state.accAvt.path}/${this.state.accAvt.name}`} alt="" />
                </div>
