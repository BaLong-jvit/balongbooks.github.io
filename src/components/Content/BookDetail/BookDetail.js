...
class BookDetail extends React.Component {
    ...
    getCommentBook() {
        ...
        return fetch(`http://localhost:4000/api/v1/books/book-comments`, {
            method: 'GET',
            headers: {
    ...
    getPublisher() {
        return fetch(`http://localhost:4000/api/v1/publishers/publisher`, {
            method: 'GET',
    ...
    getArtist() {
        return fetch(`http://localhost:4000/api/v1/artists/artists`, {
            method: 'GET',
    ...
    getImage() {
        return fetch(`http://localhost:4000/api/v1/book-images/main`, {
            method: 'GET',
            headers: { bookid: this.state.book.id }
        }).then(response => { return response.json() }).then(responseJson => {
    ...
    getSpImages() {
        return fetch(`http://localhost:4000/api/v1/book-images/support`, {
            method: 'GET',
            headers: { bookid: this.state.book.id }
        }).then(response => { return response.json() }).then(responseJson => {
    ...
    getBook() {
        return fetch(`http://localhost:4000/api/v1/books/book`, {
            method: 'GET',
    ...
    handleZoomSpImage(o) {
        let htmlString = "<img src='http://localhost:4000/image/" + o.path + "/" + o.name + "' alt='' />";
    ...
    sendComment() {
        ...
        return fetch(`http://localhost:4000/api/v1/comments/add-comment`, {
            method: 'POST',
    ...
    addToCart() {
        if (localStorage.username && localStorage.password) {
            ...
            return fetch(`http://localhost:4000/api/v1/accounts/check-login`, {
                method: 'GET',
                headers: header
            }
            ).then(response => { return response.json() }).then(responseJson => {
                if (responseJson.message === 'success') {
                    ...
                    return fetch(`http://localhost:4000/api/v1/carts/insert-book`, {
                        method: 'POST',
                        headers: {
                    ...
                    <Row>
                        <Col sm={5}>
                            <div className='view-book'>
                                <img src={`http://localhost:4000/image/${this.state.image.path}/${this.state.image.name}`} alt="" />
                            </div>
                            <div className='sp-img'>
                                {
                                    ...
                                    this.state.spImage.slice(this.state.beginSlice, this.state.beginSlice + 4).map((dplImage, key) => {
                                        return (<div className='sp-img-items' key={key} >
                                            <img src={`http://localhost:4000/image/${dplImage.path}/${dplImage.name}`} onMouseOver={() => this.handleZoomSpImage(dplImage)} alt="" />
                                        </div>)
                                    })
                                }
                                {
