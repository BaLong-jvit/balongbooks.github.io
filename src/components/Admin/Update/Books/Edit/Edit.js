...
class Edit extends Component {
    ...
    getSupImageUrls() {
        ...
        this.state.supImage.map(img => {
            arr.push({
                url: `http://localhost:4000/image/${img.path}/${img.name}`,
                id: img.id
            });
        });
    ...
    getMainImageUrl() {
        this.setState({ mainImageUrl: `http://localhost:4000/image/${this.state.mainImage.path}/${this.state.mainImage.name}` });
    }

    getCompose() {
        return fetch(`http://localhost:4000/api/v2/artists/artists`, {
            method: 'GET',
            headers: {
    ...
    getSupImage() {
        return fetch(`http://localhost:4000/api/v2/book-images/support-images`, {
            method: 'GET',
            headers: {
                bookid: this.state.book.id
            }
    ...
    getMainImage() {
        return fetch(`http://localhost:4000/api/v2/book-images/main-image`, {
            method: 'GET',
            headers: {
                bookid: this.state.book.id
            }
    ...
    getPublishers() {
        return fetch(`http://localhost:4000/api/v2/publishers/all-publishers`).then(res => { return res.json() }).then(jsonRes => {
            this.setState({ publishers: jsonRes.publishers });
        })
    }

    getArtists() {
        return fetch(`http://localhost:4000/api/v2/artists/all-artists`).then(res => { return res.json() }).then(jsonRes => {
            this.setState({ artists: jsonRes.artists });
        })
    }

    getBook() {
        return fetch(`http://localhost:4000/api/v2/books/book`, {
            method: 'GET',
            headers: { bookid: this.props.match.params.idBook }
    ...
    deleteSupImage(i) {
        let arr = [];
        if (this.state.supImageUpload !== null) {///can su ly truong hop nay
            let arrUp = [];
    ...
    updateBook() {
        ...
        return fetch(`http://localhost:4000/api/v2/books/update-book`, {
            method: 'PUT',
    ...
    updateCompose() {
        ...
        return fetch(`http://localhost:4000/api/v2/composes/update-compose`, {
            method: 'PUT',
            headers: {
    ...
    updateMainImage() {
        if (this.state.mainImageUpload !== null) {
            ...
            return fetch(`http://localhost:4000/api/v2/book-images/update-book-main`, {
                method: 'POST',
                headers: {
                    imgname: imgName,
                    username: localStorage.adminuser,
    ...
    uploadSupImage() {
        if (this.state.supImageUpload !== null) {
            ...
            return fetch(`http://localhost:4000/api/v2/book-images/delete-sup-image`, {
                method: 'DELETE',
                headers: {
                    'Content-type': 'application/json'
                },
    ...
    uploadSingleSupImage(file) {
        ...
        return fetch(`http://localhost:4000/api/v2/book-images/upload-sup-image`, {
            method: 'POST',
            headers: {
                imgname: imgname,
                username: localStorage.adminuser,
