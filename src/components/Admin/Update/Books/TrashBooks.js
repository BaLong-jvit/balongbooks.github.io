...
class TrashBooks extends Component {
    ...
    totalItemsCount() {
        return fetch(`http://localhost:4000/api/v2/books/count-trash`).then(response => { return response.json() }).then(responseJson => {
            this.setState({ totalItemsCount: (responseJson.amount.amount) });
        })
    }
    getListBook(page) {
        return fetch(`http://localhost:4000/api/v2/books/trash-books`, {
            method: 'GET',
            headers: {
                page: page
            }
